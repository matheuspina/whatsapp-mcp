  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!cleanedPhone || cleanedPhone.length < 10) {
      toast.error("Número de telefone inválido", {
        description: "Informe o código do país junto com o número, sem espaços ou símbolos.",
      });
      return;
    }
    ...
    try {
      ...
      if (result.success && result.code) {
        ...
        toast.success("Código gerado", {
          description: "Digite o código no seu celular",
        });
      } else {
      ...
      toast.error(msg.title, {
        description: msg.description,
          action: msg.action ? { label: "Configurações", onClick: () => {} } : undefined,
      });
    } finally {
      setLoading(false);
    }
    ...
    <Card className="w-full max-w-md">
      <CardHeader>
        <CardTitle className="flex items-center gap-2">
          <svg viewBox="0 0 24 24" className="w-6 h-6 text-success" fill="currentColor">
        ...
        <CardDescription>
          Informe o número que deseja conectar
        </CardDescription>
      </CardHeader>
      <CardContent>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="space-y-2">
            ...
            <p id="phone-help" className="text-sm text-muted-foreground">
              Inclua o código do país e não use zero no início.
            </p>
          </div>

...
separate SQLite database next to them.
...
```
store/messages.db  ──read-only──▶  indexer  ──writes──▶  store/index.db
...
changed rows, resolves display names, and keeps two things up to date in `store/index.db`:
...
(for backfill progress). `store/index.db` never leaves the machine; there is no network call anywhere in
this package.
